...
CREATE INDEX idx_properties_office_created ON properties(office_id, created_at DESC);

-- Properties for explore page: status + zone + type + price range
CREATE INDEX idx_properties_explore ON properties(status, zone_id, property_type_id, price);
...
CREATE INDEX idx_contact_requests_office_created ON contact_requests(office_id, created_at DESC);

-- Property images by property + sort_order
CREATE INDEX idx_property_images_property_sort ON property_images(property_id, sort_order);

-- Users by office + role (admin office user listing)
CREATE INDEX idx_users_office_role ON users(office_id, role);
