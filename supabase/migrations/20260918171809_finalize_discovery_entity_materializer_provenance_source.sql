...
begin
  ...
  if property_id is null then
    ...
    values(ptype,tx::property_transaction_type,status::property_status,title,description,
  ...
  select 'property',property_id,k,v,null::uuid, jsonb_build_object('discovery_entity_id',e.id,'source_url',e.source_url), confidence
