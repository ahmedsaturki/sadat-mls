...
export default function AqaratExploreClient({ params, initialProperties, initialCount }: Props) {
  ...
  const loadProperties = useCallback(async () => {
    ...
    try {
      ...
      const from = 0;
      const to = Math.max(11, Math.min(47, Math.max(0, count - 1)));
      const { data, count: nextCount, error: queryError } = await query.range(from, to);
  ...
  }, [count, dict.common.unexpectedError, district, maxArea, maxPrice, minArea, minPrice, propertyType, queryText, sortBy]);
